  ...
  async function initWasmModule(moduleArg = {}) {
    ...
    var MEMFS = {
      ...
      node_ops: {
        ...
        setattr(node, attr) {
          for (const key of ["mode", "atime", "mtime", "ctime"]) if (attr[key] != null) node[key] = attr[key];
    ...
    var FS = {
      ...
      open(path, flags, mode = 438) {
        ...
        } else if (isDirPath) throw new FS.ErrnoError(31);else {
      ...
      writeFile(path, data, opts = {}) {
        ...
        if (ArrayBuffer.isView(data)) FS.write(stream, data, 0, data.byteLength, undefined, opts.canOwn);else throw new Error("Unsupported data type");
        FS.close(stream);
      },
      cwd: () => FS.currentPath,
      chdir(path) {
      ...
      createStandardStreams(input, output, error) {
        if (input) FS.createDevice("/dev", "stdin", input);else FS.symlink("/dev/tty", "/dev/stdin");
        if (output) FS.createDevice("/dev", "stdout", null, output);else FS.symlink("/dev/tty", "/dev/stdout");
        if (error) FS.createDevice("/dev", "stderr", null, error);else FS.symlink("/dev/tty1", "/dev/stderr");
      ...
      forceLoadFile(obj) {
        ...
        if (typeof XMLHttpRequest != "undefined") throw new Error("Lazy loading should have been performed (contents set) in createLazyFile, but it was not. Lazy loading only works in web workers. Use --embed-file or --preload-file in emcc on the main thread.");else try {
    ...
    function ___syscall_unlinkat(dirfd, path, flags) {
      try {
        path = SYSCALLS.getStr(path);
        path = SYSCALLS.calculateAt(dirfd, path);
        if (!flags) FS.unlink(path);else if (flags === 512) FS.rmdir(path);else return -28;
        return 0;
      } catch (e) {
        if (typeof FS == "undefined" || !(e.name === "ErrnoError")) throw e;
        return -e.errno;
    ...
    function _clock_time_get(clk_id, ignored_precision, ptime) {
      ...
      var now;
      if (clk_id === 0) now = _emscripten_date_now();else if (nowIsMonotonic) now = _emscripten_get_now();else return 52;
    ...
    var getEnvStrings = () => {
      if (!getEnvStrings.strings) {
        ...
        for (var x in ENV) if (ENV[x] === undefined) delete env[x];else env[x] = ENV[x];
        var strings = [];
    ...
    cwrap(name, returnType, argTypes) {
      ...
      return (...args) => {
        ...
        const resolvedArgTypes = argTypes.map((argType, i) => {
          if (argType === "string|number") if (typeof args[i] === "number") return "number";else if (args[i]?.length > 1024) {
